...
def load_dataset(img_rows, img_cols):
    """
    split train / valid
    :param img_rows: resized image row
    :param img_cols: resized image col
    :return: normalized image numpy array (train / valid)
    """
...
def inception_module(x, filters_1x1, filters_3x3_reduce, filters_3x3, filters_5x5_reduce,\
                     filters_5x5, filters_pool_proj, name=None, kernel_init='glorot_uniform', bias_init='zeros'):
...
def main():
    ...
    x_train, y_train, x_valid, y_valid = load_dataset(224, 224)
    ...
    x = Conv2D(192, (3,3), padding='same', strides=(1,1), activation='relu', name='conv_2_3x3/1',\
               kernel_initializer=kernel_init, bias_initializer=bias_init)(x)
    x = MaxPool2D((3,3), strides=(2,2), name='max_pool_2_3x3_2', padding='same')(x)
    #Layer 3
    ...
    #Layer 4
    x = inception_module(x, 192, 96, 208, 16, 48, 64, name='inception_4a', kernel_init=kernel_init, bias_init=bias_init)
    #Auxiliary learning 1
    x1 = AveragePooling2D((5,5), strides=3, name='avg_pool_aux_1')(x)   # output size: 4x4x512
    ...
    x = GlobalAveragePooling2D(name='golbal_avg_pool_5_3x3/1')(x)
    x = Dropout(0.4)(x)
    x = Dense(10, activation='softmax', name='output')(x)

    model = Model(input_layer, [x, x1, x2], name='inception-V1')
